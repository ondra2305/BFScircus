...
class CPlanet {
public:
	CPlanet(string species, int time, bool isValid, int delay) : m_species(species),
																 m_time(time), m_valid(isValid), m_delay(delay) {}
	~CPlanet() = default;
		...
		while(!toColonise.empty()) {
            ...
            */
			...
			for(auto &neighbor : m_Graph.at(current.first)) {
				if(current.second.m_delay > current.second.m_time) {
					cout << "DELAYED:" << current.second.m_species << endl;
					toColonise.push({neighbor, {current.second.m_species, current.second.m_time + 1, true, current.second.m_delay}});
					continue;
				}
